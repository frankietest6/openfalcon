...
function storeUploadedFile(buf, claimedHash, mediaName, mimeType) {
  ...
  const m4aPath = filePath.replace(/\.bin$/, '.m4a');
    try {
      ...
      const m4aPath = filePath.replace(/\.bin$/, '.m4a');
      execSync(
        `ffmpeg -y ${inputFormat} -i "${filePath}" -vn -c:a aac -b:a 192k -movflags +faststart "${m4aPath}" 2>/dev/null`,
        { timeout: 30000 }
      );
  ...
  try {
    ...
    for (const row of rows) {
      ...
      try {
        ...
        execSync(
          `ffmpeg -y ${inputFormat} -i "${binPath}" -vn -c:a aac -b:a 192k -movflags +faststart "${m4aPath}" 2>/dev/null`,
          { timeout: 60000 }
        );
        if (fs.existsSync(m4aPath) && fs.statSync(m4aPath).size > 1000) {
