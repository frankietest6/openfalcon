...
// Seed built-in viewer templates (v0.32.0+)
// Five themed templates ship in public/viewer-templates/ and get inserted
// the first time we see each by name. Idempotent: if an admin has manually
// created a template with the same name, we leave it alone. If the admin
// deletes a built-in by name, we re-create it on next boot (built-ins are
// a "preview catalog" — to hide one permanently, rename it).
//
// DISPLAY_NAMES has explicit entries for the slugs we ship, falling back
// to a generic title-case for any future file dropped into the directory.
{
  const fs = require('fs');
  const path = require('path');
  ...
  for (const file of files) {
    ...
    const existing = db.prepare(
      `SELECT id FROM viewer_page_templates WHERE name = ? LIMIT 1`
    ).get(name);
